...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN books ON series.id = books.series_id JOIN character_books ON books.id = character_books.book_id JOIN characters ON character_books.character_id = characters.id GROUP BY characters.id, series.title COUNT(characters.species) HAVING characters.species = 'human');"
end
